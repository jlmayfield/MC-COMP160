...
Imagine a program that animates a ball bouncing around a rectangular space.  We'll use \(x\) and \(y\) to represent the ball's current location in the space\sidenote{Remember (0,0) is the upper left hand corner} and \(dx\) and \(dy\) to represent the distance it moves in the x and y directions, respectively, in a single frame of animation.
